    ...
    public function test_owner_views_management_dashboard_with_real_phase_six_metrics(): void
    {
        ...
        $this->actingAs($owner)->get('/app/dashboard')
            ...
            ->assertInertia(fn (AssertableInertia $page) => $page
                ...
                ->where('dashboardDate', '2026-06-16')
                ->where('topStats.0.key', 'revenue')
                ->where('topStats.0.value', 100000)
                ->where('topStats.1.key', 'overdue')
                ->where('topStats.1.value', 75000)
                ->where('topStats.2.key', 'active_contracts')
                ->where('topStats.2.value', 1)
                ->where('topStats.3.key', 'attendance_rate')
                ->where('topStats.3.value', 50)
                ->where('topStats.4.key', 'today_visits')
                ->where('topStats.4.value', 2)
                ->where('visitTimeline.0.site', $context['contract']->site->name)
                ->where('visitTimeline.0.worker_count', 1)
                ->where('visitTimeline.0.timeline_start_percent', 0)
                ->where('visitTimeline.1.timeline_start_percent', 50)
                ->where('invoiceSummary.total_halalas', 100000)
                ->where('invoiceSummary.paid_halalas', 25000)
                ->where('invoiceSummary.open_halalas', 75000)
                ->where('invoiceSummary.overdue_halalas', 75000)
                ->where('overdueInvoices.0.number', 'INV-A6-001')
                ->where('overdueInvoices.0.days_overdue', 6)
                ->where('onlineWorker.name', 'A6 Worker')
                ->where('onlineWorker.current_visit.site', $context['contract']->site->name)
            );
